...
test("free AKT questions page exists with tracked free CTA and required SEO copy", () => {
  ...
  assert.match(heroLoop, /free-akt-hero-readable-loop\.webm/);
  assert.match(heroLoop, /free-akt-hero-readable-loop\.mp4/);
  assert.match(heroLoop, /free-akt-hero-readable-poster\.jpg/);
  assert.match(heroLoop, /AKT-style SBA, structured explanation, clue trap why wrong, and adaptive next session/);
  assert.equal(fs.existsSync("public/video/free-akt-hero-readable-loop.webm"), true);
  assert.equal(fs.existsSync("public/video/free-akt-hero-readable-loop.mp4"), true);
  assert.equal(fs.existsSync("public/video/free-akt-hero-readable-poster.jpg"), true);
